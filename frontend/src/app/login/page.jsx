...
import { Eye, EyeOff, Bug, UserCog } from "lucide-react";
...
const LoginForm = () => {
  ...
  const loginForm = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        // ✅ Store both the userId and the token for authentication
        localStorage.setItem("userId", res.data.user._id);
        localStorage.setItem("token", res.data.token);
        resetForm();
        // ✅ Add a delay to the redirect to allow localStorage to update
        setTimeout(() => {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/users/google-auth`, {
          token: tokenResponse.access_token,
        });
        console.log("Google login success:", res.data);
        toast.success("Google sign-in successful! ✅", { duration: 3000 });
        // ✅ Store both the userId and the token for authentication
        localStorage.setItem("userId", res.data.user._id);
        localStorage.setItem("token", res.data.token);
        // ✅ Add a delay to the redirect to allow localStorage to update
        ...
        toast.error("Google sign-in failed. ❌", { duration: 3000 });
      }
    },
    onError: () => toast.error("Google sign-in failed. ❌", { duration: 3000 }),
  });
          ...
          <div className="text-center mb-6">
            ...
            <p className="mt-2 text-sm text-neutral-400">
              ...
              <Link href="/admin/dashboard">
                <span className="text-blue-500 decoration-2 hover:underline font-medium cursor-pointer flex items-center justify-center gap-1 mt-2">
